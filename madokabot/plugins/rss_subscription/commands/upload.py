...
from ..download_cleanup import cancel_download, delete_download_files
from ..download_records import get_download_record_messages
from ..download_state import DownloadStatus, download_tasks
from ..download_validation import DownloadSizeLimitExceeded
from ..uploads.service import retry_upload_to_group
...
async def _wait_for_file_selection(
    ...
    *,
    ...
    while (remaining := deadline - time.monotonic()) > 0:
        ...
        if (
            task_info is None
            or task_info.get("status") != DownloadStatus.WAITING_SELECTION
        ):
            return
        try:
...
async def handle_upload_file(
    ...
    target = _extract_download_url(content.result if content.available else "")
    if not target:
...
@rss_retry_upload_cmd.got("UPLOAD_RETRY_GID", prompt="请输入要重试上传的任务 GID")
...
@rss_delete_file_cmd.got("DELETE_FILE_GID", prompt="请输入要删除下载文件的任务 GID")
async def handle_delete_files(
    ...
    try:
        ...
        deleted_count, failed_paths = await delete_download_files(gid, group_id)
    except Aria2Error as exc:
