...
from typing import Any, List, Match as TypingMatch, Optional
...
from ..subscription import DuplicateFilterMode, Rss
from ..utils import regex_validate
from .matchers import rss_edit_cmd


@rss_edit_cmd.handle()
async def prepare_rss_change(matcher: Matcher, content: AlcMatch[str]) -> None:
...
def handle_time_change(value_to_change: str) -> str:
    if not re.search(r"[_*/,-]", value_to_change):
        return str(max(int(float(value_to_change)), 1))
...
def handle_change_list(
    ...
    if key_to_change == "name":
        ...
        value_to_change = handle_property(
            value_to_change, getattr(rss, ATTRIBUTE_MAP[key_to_change])
        )  # type: ignore
    ...
    setattr(rss, ATTRIBUTE_MAP[key_to_change], value_to_change)
...
注：
    ...
    2. proxy/ot/op/ohp/downopen/upgroup/stop 值为 1/0
...
async def filter_rss_by_permissions(
    ...
    if guild_channel_id:
        if re.search(" (qq|qun|channel)=", change_info):
            await rss_edit_cmd.finish(
                "❌ 禁止在子频道中修改订阅账号！如要取消订阅请使用 RSS 删除命令！"
            )
        rss_list = [
            rss
            for rss in rss_list
...
async def handle_rss_change(
    ...
    if len(rss_list) > 1 and " name=" in change_info:
        await rss_edit_cmd.finish("❌ 禁止将多个订阅批量改名！会因为名称相同起冲突！")

    if " name=" in f" {change_info}" or change_info.startswith("name="):
        name_value = next(
            (
                value
                for part in change_info.split()
        ...
        if name_value is not None:
            if not name_value:
                await rss_edit_cmd.finish("❌ 订阅名不能为空！")
            existing = Rss.get_one_by_name(name_value)
    ...
    await rss_edit_cmd.finish(result_msg)
...
async def validate_rss_change(key_to_change: str, value_to_change: str) -> None:
    ...
    if key_to_change == "mode" and (
        ...
        "stop",
        "forward",
    } and value_to_change not in {"0", "1"}:
        await rss_edit_cmd.finish(
            f"❌ {key_to_change} 只能设置为 1 或 0！\n{key_to_change}={value_to_change}"
        )
    elif key_to_change == "time":
        try:
            if not re.search(r"[_*/,-]", value_to_change):
                minutes = float(value_to_change)
                if not math.isfinite(minutes) or minutes < 1:
                    raise ValueError
        except (TypeError, ValueError, OverflowError):
            await rss_edit_cmd.finish(
                f"❌ 更新频率必须是分钟数或合法 cron 格式！\ntime={value_to_change}"
            )
    elif key_to_change == "img_num":
        try:
            if int(value_to_change) < 0:
                raise ValueError
        except ValueError:
            await rss_edit_cmd.finish(
                f"❌ 图片数量限制必须是非负整数！\nimg_num={value_to_change}"
            )
...
async def batch_change_rss(
    ...
    guild_channel_id: Optional[str],
    rss_list: List[Rss],
    rm_list_exist: Optional[TypingMatch[str]] = None,
) -> List[Rss]:
    changed_rss_list = []
...
def handle_rm_list(
    ...
    if rm_list_exist:
        rm_list_str = rm_list_exist[0].split("=", 1)[1]
        if rm_list_str == "-1":
            rm_list = ["-1"]
        else:
