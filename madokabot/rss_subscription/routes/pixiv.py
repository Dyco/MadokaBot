...
# 如果启用了去重模式，对推送列表进行过滤
@HandlerRegistry.append_before_handler(rex="/pixiv/", priority=12)
...
# 处理图片
@HandlerRegistry.append_handler(parsing_type="picture", rex="/pixiv/")
...
async def get_ugoira_video(ugoira_id: str) -> Any:
    async with aiohttp.ClientSession() as session:
        ...
        resp = await session.post(
            "https://ugoira.huggy.moe/api/illusts",
            data=data,
            proxy=get_proxy(),
        )
...
# 处理来源
@HandlerRegistry.append_handler(parsing_type="source", rex="/pixiv/")
