...
from .json_data import JsonData, JsonDataStore

GROUP_SET_PATH = assets.get_dir(ResType.JSON, SubFolder.GROUP) / "group_set.json"
...
class GroupSetStore:
    ...
    def __init__(self, save_path: Path = GROUP_SET_PATH) -> None:
        """初始化群组配置存储。"""
    ...
    def set(
        ...
        normalized_group_id = self._normalize_group_id(group_id)
        normalized_plugin_name = self._normalize_plugin_name(plugin_name)
        self.content.setdefault(normalized_group_id, {})[
            normalized_plugin_name
        ] = data
        self.save()
...
group_set = GroupSetStore()
# 通用群组配置存储实例，插件只需传入群 ID 和插件名即可使用。


__all__ = ["GROUP_SET_PATH", "GroupSetStore", "group_set"]
